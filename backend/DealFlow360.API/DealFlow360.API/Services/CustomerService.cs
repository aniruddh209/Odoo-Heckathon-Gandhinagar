...
public interface ICustomerService
{
    ...
    Task<CustomerQuoteDto> AcceptRepCounterOfferAsync(int customerId, int quotationId, string? remarks = null);
    Task<CustomerQuoteDto> RejectRepCounterOfferAsync(int customerId, int quotationId, RejectRepCounterRequest request);
...
public class CustomerService : ICustomerService
{
    ...
    public async Task<CustomerQuoteDto> AcceptRepCounterOfferAsync(int customerId, int quotationId, string? remarks = null)
    {
        ...
        _context.QuotationChanges.Add(new QuotationChange
        {
            ...
            Description = $"Customer {quotation.Customer?.Name} accepted the sales representative's counter-offer. {(remarks ?? "")}".Trim(),
            RequestedByUserId = quotation.SalesRepId,
            CreatedAtUtc = DateTime.UtcNow
        });
    ...
    public async Task<CustomerQuoteDto> RejectRepCounterOfferAsync(int customerId, int quotationId, RejectRepCounterRequest request)
    {
        var quotation = await _context.Quotations
            .Include(q => q.Customer).ThenInclude(c => c.Tier)
            .Include(q => q.Lines).ThenInclude(l => l.Product)
        ...
        _context.QuotationChanges.Add(new QuotationChange
        {
            ...
            Description = $"Customer {quotation.Customer?.Name} declined the counter-offer. Note: {request.Reason ?? "Customer requested further discussion."}",
            RequestedByUserId = quotation.SalesRepId,
