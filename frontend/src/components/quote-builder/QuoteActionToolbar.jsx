...
import { QuotationStatus } from '../../constants/index.js';
...
export const QuoteActionToolbar = ({
  quotation,
  onSubmitForApproval,
  onSendToCustomer,
  onConvertToOrder,
  ...
  const status = quotation.Status || quotation.status;
  const isDraft = status === QuotationStatus.Draft;
  const isInReview = status === QuotationStatus.InReview || status === QuotationStatus.PendingApproval;
  const isApproved = status === QuotationStatus.Approved;
  const isSent = status === QuotationStatus.SentToCustomer || status === QuotationStatus.Sent;
  const isAccepted = status === QuotationStatus.Accepted || status === QuotationStatus.Confirmed;
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex items-center space-x-3">
        <div className="flex items-center space-x-1.5 text-sm text-slate-500">
          ...
          <span>Version: <strong className="text-slate-700">v{quotation.VersionNumber || quotation.versionNumber || 1}</strong></span>
        </div>
        {(quotation.ExpirationDate || quotation.expirationDate) && (
          <>
            <span className="text-slate-300">•</span>
            <div className="flex items-center space-x-1 text-xs text-slate-500">
              <Clock className="w-3.5 h-3.5 text-slate-400" />
              <span>Expires: {new Date(quotation.ExpirationDate || quotation.expirationDate).toLocaleDateString()}</span>
            </div>
          </>
        )}
      </div>
      ...
      <div className="flex items-center space-x-2">
        ...
        {isDraft && (
          <>
            {quotation.ApprovalRequired || quotation.approvalRequired ? (
              <Button
                variant="primary"
                size="sm"
