...
export const ConnectSalesSection = ({ profile, onConnectionCreated, onNavigateToInquiries }) => {
  ...
  const handleSelectCompany = (company) => {
    if (selectedCompany?.id === company.id) {
      // Toggle / deselect if clicking the same one again to see all products
    ...
    // If current selectedProduct does not belong to this company, reset it
    if (selectedProduct && selectedProduct.companyId !== company.id) {
  ...
  const filteredProducts = products.filter(p => {
    if (selectedCompany && p.companyId && p.companyId !== selectedCompany.id) {
      return false;
    }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs overflow-hidden">
      ...
      {/* Dedicated Account Executive Banner (if assigned) */}
      {profile?.assignedSalesRepName && (
        <div className="p-5 bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-slate-50 border-b border-indigo-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-xl bg-indigo-600 text-white flex items-center justify-center font-bold text-base shadow-xs shrink-0">
              {profile.assignedSalesRepName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-slate-900">{profile.assignedSalesRepName}</h3>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 font-semibold">
                  Dedicated Account Executive
                </span>
              </div>
              <p className="text-xs text-slate-600 mt-0.5">
                Primary Commercial Executive for <strong>{profile.name || 'Your Enterprise'}</strong> ({profile.tierName || 'Standard'} Tier)
              </p>
          ...
          <div className="flex items-center gap-2">
            <a
              href={`mailto:${profile.assignedSalesRepEmail || 'sales@dealflow360.com'}?subject=Direct%20Inquiry%20from%20${encodeURIComponent(profile.name || 'Customer')}`}
              className="px-3.5 py-2 text-xs font-semibold rounded-lg bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 transition-colors shadow-2xs inline-flex items-center gap-1.5 shrink-0"
            >
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border text-left cursor-pointer transition-all ${
                    isSelected
                      ? 'border-blue-600 bg-blue-50/60 shadow-xs ring-2 ring-blue-600'
        ...
        <section>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {selectedCompany 
                  ? `Showing verified catalog products for ${selectedCompany.name}`
          ...
          {isLoadingCatalog ? (
            ...
            <div className="text-center py-8 text-xs text-slate-400 border border-dashed border-slate-200 rounded-xl bg-slate-50">
