...
export const OneTimeInvoiceCard = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs space-y-4 hover:border-slate-300 transition-colors">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center space-x-2">
            <FileText className="w-4 h-4 text-blue-600" />
            <span className="font-bold text-slate-800 text-sm font-mono">{invoice?.InvoiceNumber}</span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 py-2 border-y border-slate-100 text-xs">
        ...
        <div className="text-right">
          ...
          <span className="font-mono text-base font-bold text-slate-900">
            {formatCurrency(invoice?.TotalAmount ?? invoice?.totalAmount ?? 0, invoice?.Currency || 'INR')}
          </span>
        </div>
      </div>
        ...
        <div className="space-y-1.5 text-xs text-slate-600">
          ...
          {invoice.Lines.slice(0, 3).map((line) => (
            <div key={line.Id || line.id} className="flex justify-between">
              ...
              <span className="font-mono text-slate-700">{formatCurrency(line.Total ?? line.total ?? 0, invoice?.Currency || 'INR')}</span>
            </div>
          ))}
      ...
      <div className="flex items-center justify-end space-x-2 pt-2">
        ...
        {!isPaid && onRecordPayment && (
          <Button size="sm" onClick={() => onRecordPayment(invoice.Id)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">
...
export default OneTimeInvoiceCard;
