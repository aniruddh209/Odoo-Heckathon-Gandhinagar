    ...
    <div className="bg-white rounded-xl border border-slate-200/80 p-6 shadow-xs space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span
                className={`text-xs px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border ${
                  isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200/80' : 'bg-slate-100 text-slate-600 border-slate-200/80'
                }`}
              >
                {subscription.Status}
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5 font-mono">Contract: {subscription.SubscriptionNumber}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {onCancel && isActive && (
            <Button size="sm" variant="outline" onClick={() => onCancel(subscription.Id)} className="text-rose-600 hover:bg-rose-50 border-rose-200/80">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
        ...
        <div className="p-3 bg-slate-50/70 rounded-lg border border-slate-100">
          ...
          <span className="font-mono font-bold text-slate-900 text-sm">
            {formatCurrency(subscription.PeriodicPrice ?? subscription.periodicPrice ?? 0, subscription.Currency || 'INR')} / {subscription.BillingFrequency === 'Annual' ? 'yr' : 'mo'}
          </span>
        </div>
        <div className="p-3 bg-slate-50/70 rounded-lg border border-slate-100">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-slate-700">
                    {sch.ScheduledDate ? formatDate(sch.ScheduledDate) : 'N/A'}
                  </span>
                  <span className="text-slate-400">({sch.PeriodDescription})</span>
                </div>
                <div className="flex items-center space-x-3">
                  <span className="font-mono font-bold text-slate-800">{formatCurrency(sch.Amount ?? sch.amount ?? 0, subscription.Currency || 'INR')}</span>
                  <span
