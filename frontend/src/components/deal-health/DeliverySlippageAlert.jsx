...
export const DeliverySlippageAlert = ({
  orderNumber,
  customerName,
  promisedDate,
  revisedDate,
    ...
    <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs space-y-2">
      ...
      <div className="flex items-center justify-between pt-1 border-t border-amber-200/60 text-slate-500">
        <div className="flex items-center space-x-3">
          <span>Promised: <strong className="text-slate-700">{promisedDate ? new Date(promisedDate).toLocaleDateString() : 'N/A'}</strong></span>
          <span>→</span>
          <span>Revised: <strong className="text-rose-600">{revisedDate ? new Date(revisedDate).toLocaleDateString() : 'N/A'}</strong></span>
        </div>
