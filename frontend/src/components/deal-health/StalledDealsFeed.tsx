...
import { DealAlertDto } from '../../types';
import { Button } from '../common/Button';
import { AlertCircle, Clock, User, ArrowRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

interface StalledDealsFeedProps {
  alerts: DealAlertDto[];
  onNudgeRep?: (dealId: string) => void;
}

export const StalledDealsFeed: React.FC<StalledDealsFeedProps> = ({
  alerts,
  onNudgeRep,
}) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        {alerts.map((deal) => (
          <div key={deal.DealId} className="p-4 hover:bg-slate-50/70 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2 shrink-0">
              {onNudgeRep && (
                <Button size="sm" variant="outline" onClick={() => onNudgeRep(deal.DealId)}>
                  Nudge Rep
                </Button>
              )}
              <Button
                size="sm"
                variant="ghost"
                onClick={() => navigate(`/quotations/${deal.DealId}`)}
              >
                Inspect
