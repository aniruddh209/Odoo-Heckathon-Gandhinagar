...
import { ApprovalRequestDto } from '../types';
...
export const ApprovalDetailPage: React.FC = () => {
  ...
  // Pick active request
  const activeRequest: ApprovalRequestDto | null =
    requests.find((r: ApprovalRequestDto) => String(r.Id ?? r.id) === String(selectedRequestId || id)) ||
    (requests.length > 0 ? requests[0] : null);

  const reqQuoteId = activeRequest?.QuotationId ?? activeRequest?.quotationId;

  // If we have an active request, also fetch full quotation details
  const { data: quotation } = useQuery({
    queryKey: ['quotation', reqQuoteId],
    queryFn: () => quotationApi.getQuotation(reqQuoteId!),
    enabled: !!reqQuoteId,
  });

  const decisionMutation = useMutation({
    mutationFn: ({ reqId, action, comments }: { reqId: string; action: 'Approve' | 'Reject'; comments?: string }) =>
      approvalApi.recordDecision(reqId, { Action: action, Comments: comments }),
    onSuccess: (_, variables) => {
      ...
      setAlertMessage({
        type: 'success',
        text: `Quotation has been successfully ${variables.action.toLowerCase()}d.`,
      });
    },
    onError: (err: any) => {
      setAlertMessage({
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
            {isLoadingRequests ? (
              ...
              requests.map((req: ApprovalRequestDto) => {
      ...
      {activeRequest && (
        <ApprovalDecisionModal
          ...
          onConfirm={(action, comments) => {
            ...
            decisionMutation.mutate({ reqId: String(reqId), action, comments });
          }}
        />
      )}
    </div>
