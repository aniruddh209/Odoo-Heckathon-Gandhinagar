...
export const QuotationBuilderPage = () => {
  ...
  const handleAddRecommendation = (rec) => {
    ...
    if (existingIdx >= 0) {
      setLines((prev) =>
        prev.map((l, idx) =>
          idx === existingIdx ? { ...l, quantity: (parseInt(l.quantity, 10) || 1) + 1 } : l
        )
      );
    } else {
      const newLine = {
        ...
        discountPercent: orderDiscountPercent ? parseFloat(orderDiscountPercent) : 0,
      };
      setLines((prev) => [...prev, newLine]);
    }
    toast.success('Recommendation Added', `Added ${rec.productName} to deal.`);
  };

  const handleDismissRecommendation = (productId) => {
    setDismissedRecs((prev) => ({ ...prev, [productId]: true }));
  };
  ...
  const visibleRecommendations = recommendations.filter((r) => !dismissedRecs[r.productId]);
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-4 sticky top-20">
          ...
          {/* Live Upsell & Cross-Sell Recommendations Panel */}
          {visibleRecommendations.length > 0 && (
            <div className="p-4 bg-gradient-to-br from-indigo-50/60 via-purple-50/40 to-white rounded-xl border border-indigo-200/80 shadow-xs space-y-3">
              ...
              <div className="space-y-2.5 max-h-80 overflow-y-auto pr-1">
                {visibleRecommendations.map((rec) => (
                  <div
                    key={rec.productId}
                    className="p-3 bg-white rounded-lg border border-indigo-100 shadow-2xs hover:border-indigo-300 transition-all space-y-2"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 font-mono mt-0.5">
                          SKU: {rec.sku} • {formatCurrency(rec.unitPrice, currencyCode)}
                        </p>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="flex items-center gap-2 text-[11px]">
                      <span className="inline-flex items-center gap-1 font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200/60">
                        <TrendingUp className="w-3 h-3 text-emerald-600" />
                        {rec.marginDeltaPercent >= 0 ? `+${rec.marginDeltaPercent}%` : `${rec.marginDeltaPercent}%`} Margin
                      </span>
                      <span className="text-slate-500">
                        Deal Margin: {rec.marginAfterAddition}%
                      </span>
                    </div>

                    {rec.reason && (
                      <p className="text-[11px] text-slate-600 italic bg-slate-50/80 p-1.5 rounded border border-slate-100">
                        "{rec.reason}"
                      </p>
                    )}
                    ...
                    >
                      Add to Quote (Recalculate Margin)
                    </Button>
                  </div>
                ))}
              </div>
