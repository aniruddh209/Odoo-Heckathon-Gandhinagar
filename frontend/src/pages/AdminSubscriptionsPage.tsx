...
export const AdminSubscriptionsPage: React.FC = () => {
  ...
  const [periodicPrice, setPeriodicPrice] = useState<number>(0);
  ...
  const createMutation = useMutation({
    mutationFn: (newPlan: Partial<SubscriptionPlanDto>) => billingApi.createSubscriptionPlan(newPlan),
    onSuccess: () => {
    ...
    <div className="space-y-6 animate-in fade-in">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {isLoading ? (
                ...
                plans.map((p: SubscriptionPlanDto) => {
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create Subscription Plan" size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Plan Name" placeholder="e.g. Enterprise SLA 24/7 Gold" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} required />
          <Select
          ...
          <Input
            ...
            step="0.01"
            value={periodicPrice}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPeriodicPrice(parseFloat(e.target.value) || 0)}
            required
          />
          <Input
