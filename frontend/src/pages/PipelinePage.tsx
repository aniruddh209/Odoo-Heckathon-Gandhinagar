...
export const PipelinePage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pipeline-quotations'],
    queryFn: () => quotationApi.getQuotations({ PageNumber: 1, PageSize: 100 }),
  });
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 overflow-x-auto pb-4">
          {COLUMNS.map((col) => {
            const columnQuotes = quotes.filter((q: QuotationDto) => {
              const st = q.Status || q.status;
              return st ? col.statuses.includes(st as any) : false;
            });
            const columnTotal = columnQuotes.reduce((sum: number, q: QuotationDto) => sum + (q.TotalAmount ?? q.totalNetAmount ?? 0), 0);
              ...
              <div key={col.id} className="flex flex-col bg-slate-100/70 rounded-xl p-3 border border-slate-200 min-w-[260px]">
                ...
                <div className="space-y-3 mt-2 flex-1">
                  {columnQuotes.length === 0 ? (
                    ...
                    columnQuotes.map((q: QuotationDto) => (
                      <div
                        key={q.Id}
