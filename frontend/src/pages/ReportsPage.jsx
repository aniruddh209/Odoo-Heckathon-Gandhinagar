...
export const ReportsPage = () => {
  ...
  useEffect(() => {
    ...
    reportApi.getRepPerformance()
      .then((data) => {
        if (isMounted) setRepPerformance(data || []);
      })
      .catch((err) => console.error('Error fetching rep performance:', err))
      .finally(() => {
                ...
                repPerformance.map((rep) => (
                  <tr key={rep.RepId} className="hover:bg-slate-50/70">
                    ...
                    <td className="py-3.5 px-4 text-right font-mono font-bold text-slate-900">
                      ${(rep.TotalBookedRevenue || 0).toLocaleString()}
                    </td>
                    <td className="py-3.5 px-4 text-right font-mono text-amber-600">
                      {(rep.AverageDiscountPercentage || 0).toFixed(1)}%
                    </td>
                    <td className="py-3.5 px-4 text-right">
                      <span
                        className={`inline-flex items-center text-xs font-bold px-2 py-0.5 rounded-full ${
                          (rep.AverageMarginPercentage || 0) >= 25
                            ? 'bg-emerald-50 text-emerald-700'
                            : 'bg-rose-50 text-rose-700'
                        }`}
                      >
                        {(rep.AverageMarginPercentage || 0).toFixed(1)}%
                      </span>
                    </td>
                  </tr>
                ))
