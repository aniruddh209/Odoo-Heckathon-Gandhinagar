...
export const AdminProductsPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newProd: Partial<ProductDto>) => productApi.createProduct(newProd),
    onSuccess: () => {
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Product SKU" size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          <Input label="SKU Code" placeholder="e.g. HW-SRV-001" value={sku} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSku(e.target.value)} required />
          <Input label="Product Name" placeholder="e.g. Enterprise Cloud Server Node" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} required />
          <Input label="Description" placeholder="Specifications, warranty, details..." value={description} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDescription(e.target.value)} />

          <div className="grid grid-cols-2 gap-4">
            <Input label="Cost Price ($)" type="number" step="0.01" value={costPrice} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCostPrice(parseFloat(e.target.value) || 0)} required />
            <Input label="Base Price ($)" type="number" step="0.01" value={basePrice} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBasePrice(parseFloat(e.target.value) || 0)} required />
          </div>
