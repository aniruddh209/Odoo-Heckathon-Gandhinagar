...
import { WarehouseDto, WarehouseStockDto } from '../types';
...
export const AdminWarehousesPage: React.FC = () => {
  ...
  const activeWarehouse: WarehouseDto | null =
    warehouses.find((w: WarehouseDto) => w.Id === selectedWarehouseId) ||
    (warehouses.length > 0 ? warehouses[0] : null);

  const { data: stocks = [], isLoading: isLoadingStocks } = useQuery({
    queryKey: ['warehouse-stocks', activeWarehouse?.Id],
    queryFn: () => fulfillmentApi.getWarehouseStocks(activeWarehouse!.Id),
    enabled: !!activeWarehouse?.Id,
  });
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {isLoadingStocks ? (
                  ...
                  stocks.map((s: WarehouseStockDto) => (
