...
import { QuotationStatus, Role } from '../types';
...
export const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    quotationApi.getQuotations({ PageNumber: 1, PageSize: 6 })
      .then((data) => {
        if (isMounted) setQuotesData(data);
      })
      .catch((err) => console.error('Error fetching dashboard quotes:', err))
      .finally(() => {
        if (isMounted) setIsLoadingQuotes(false);
      });

    healthApi.getDealAlerts()
      .then((data) => {
        if (isMounted) setAlerts(data || []);
      })
      .catch((err) => console.error('Error fetching deal alerts:', err));

    if (hasRole([Role.SalesManager, Role.FinanceOperations, Role.Admin])) {
      healthApi.getRepAnomalies()
        .then((data) => {
          if (isMounted) setAnomalies(data || []);
        })
        .catch((err) => console.error('Error fetching rep anomalies:', err));
    }
  ...
  // Derived metrics
  const totalPipeline = quotes.reduce((acc, q) => acc + (q.TotalAmount ?? q.totalNetAmount ?? 0), 0);
  const avgMargin = quotes.length
    ? quotes.reduce((acc, q) => acc + (q.OrderGrossMarginPercent ?? 28), 0) / quotes.length
    : 0;
